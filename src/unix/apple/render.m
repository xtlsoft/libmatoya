...
static bool render_init_api(MTY_Renderer *ctx, MTY_GFX api, id<MTLDevice> device)
{
	switch (api) {
		case MTY_GFX_METAL:
			return !ctx->metal ? gfx_metal_init(device, &ctx->metal) : true;
		case MTY_GFX_GL:
	...
	const void *image, const MTY_RenderDesc *desc, MTY_Texture *dest)
{
	...
	if (!render_init_api(ctx, api, cq.device))
		return false;
	...
	switch (ctx->api) {
		case MTY_GFX_METAL:
			gfx_metal_render(ctx->metal, cq,
				image, desc, (__bridge id<MTLTexture>) dest);
			return true;
		case MTY_GFX_GL:
